...
import {COUNT_OF_SIMULAR_FILMS, Page, Tab} from '../../const';
        ...
        <div className="movie-card__wrap">
          <div className="movie-card__desc">
            ...
            <div className="movie-card__buttons">
              ...
              {useAuthtorization() ? <Link to={generatePath(path, {id, tab: Page.REVIEW})} className="btn movie-card__button">Add review</Link> : ``}
            </div>
          </div>
        </div>
      </div>
            ...
            <nav className="movie-nav movie-card__nav">
              <ul className="movie-nav__list">
                ...
                <li className={`movie-nav__item ${tab === Tab.DETAILS ? `movie-nav__item--active` : ``}`}>
                  <Link to={generatePath(path, {id, tab: Tab.DETAILS})} className="movie-nav__link">Details</Link>
                </li>
                <li className={`movie-nav__item ${tab === Tab.REVIEWS ? `movie-nav__item--active` : ``}`}>
                  <Link to={generatePath(path, {id, tab: Tab.REVIEWS})} className="movie-nav__link">Reviews</Link>
                </li>
              </ul>
            </nav>
